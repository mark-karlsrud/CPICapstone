...
public class InteractionManager : MonoBehaviour 
{
	...
	public Texture gripHandTexture2;
	public Texture releaseHandTexture2;
	public static bool usedOnce = false;
	...
	private Vector3 cursor2ScreenPos = Vector3.zero;
		...
		if(kinectManager && kinectManager.IsInitialized())
		{
			...
			if(primaryUserID != 0)
			{
				...
				//{
					...
					{
						cursor2ScreenPos = Vector3.Lerp(cursor2ScreenPos, rightHandScreenPos, smoothFactor * Time.deltaTime);					}
					//					else
					//					{
					...
					if(controlMouseCursor && !useHandCursor)
					{
						MouseControl.MouseMove(cursor2ScreenPos, debugText);
					}
				//}
				else
				{
	...
	void OnGUI()
	{
		...
		if (!EndSceaneGUI.turnOff) {
			...
			if (debugText) {
				...
				//{
				sGuiText += "Cursor: " + cursor2ScreenPos.ToString ();
			...
			if (useHandCursor) {
				...
				//if(isLeftHandPrimary)
				//{
				//if (lastLeftHandEvent == HandEventType.Release)
				//texture = releaseHandTexture;
				//}
				//else if(isRightHandPrimary)
				//{
				if (lastRightHandEvent == HandEventType.Grip)
					texture2 = gripHandTexture2;
				else if (lastRightHandEvent == HandEventType.Release)
					texture2 = releaseHandTexture2;
				//}

				showTexture = lastLeftHandEvent == HandEventType.Grip;
				if (texture == null && !showTexture) {
					texture = normalHandTexture;
				}
				if (texture2 == null) {
					texture2 = normalHandTexture2;
				}
				...
				if (useHandCursor) {
					...
					if ((texture2 != null)) {//&& (isLeftHandPrimary || isRightHandPrimary))
						//handCursor.transform.position = cursorScreenPos; // Vector3.Lerp(handCursor.transform.position, cursorScreenPos, 3 * Time.deltaTime);
						Rect rectTexture2 = new Rect (cursor2ScreenPos.x * Screen.width - texture2.width / 2, (1f - cursor2ScreenPos.y) * Screen.height - texture2.height / 2,
					                             texture2.width, texture2.height);
						GUI.DrawTexture (rectTexture2, texture2);
